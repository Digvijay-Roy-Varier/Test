...
void TrainAcceptRejectEPMVA(TTree* mytree)                  //*  ,bool calo=false)
{
  ...
  TString outfileName;
  //if(calo)
  outfileName = TString("TrainAcceptRejectEP.root");
  //else
  ...
  //if(calo)
  dataloader = new TMVA::DataLoader("TrainAcceptRejectEPMVA");
  //else
  ...
  dataloader->AddVariable("edep", "Deposited energy", "MeV",'F');
  dataloader->AddVariable("edep/sqrt(pow(2.5,2)-pow(mcshd,2))", "Energy deposited per unit length ","MeV",'F');
  ...
  */
  ...
  TCut signalCut = "mcpdg==11&&mcproc==56&&mcom<110&&mcom>80";
  TCut backgrCut = "mcpdg==2212";
